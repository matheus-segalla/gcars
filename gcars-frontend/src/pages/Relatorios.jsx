...
export default function Relatorios() {
  ...
  return (
    <div className="space-y-8">
      ...
      {loadingStats ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
            ...
            {/* Faturamento */}
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 sm:p-5 space-y-2 shadow-xl transition-colors">
            ...
            {/* Lucro Líquido Real */}
            <div className="col-span-2 sm:col-span-1 bg-emerald-500/10 dark:bg-gradient-to-b dark:from-emerald-950/40 dark:to-zinc-900 border border-emerald-500/30 dark:border-emerald-500/40 rounded-2xl p-4 sm:p-5 space-y-2 shadow-2xl relative overflow-hidden transition-colors">
              <div className="flex items-center justify-between text-emerald-600 dark:text-emerald-400">
                <span className="text-[10px] sm:text-xs font-extrabold uppercase tracking-wider">Lucro Real</span>
                <span className="text-[10px] font-bold bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 px-2 py-0.5 rounded-full border border-emerald-500/30 flex items-center gap-0.5">
              ...
              <span className="text-[10px] text-emerald-600 dark:text-emerald-500/80 block font-medium">Margem Líquida</span>
            </div>
      ...
      {/* 📋 LISTA PAGINADA DE ORDENS COM CUSTOS */}
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 sm:p-6 shadow-xl space-y-5 transition-colors">
        
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-zinc-200 dark:border-zinc-800/80 pb-4">
          <div>
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-red-500">Gestão & Auditoria</span>
            <h3 className="text-base sm:text-lg font-black text-zinc-900 dark:text-white flex items-center gap-2 mt-0.5">
              <FileSpreadsheet className="w-5 h-5 text-red-500" /> Últimos Serviços & Margem Real
            </h3>
          </div>
          <span className="text-xs text-zinc-600 dark:text-zinc-400 font-medium bg-zinc-100 dark:bg-zinc-950 px-3 py-1.5 rounded-xl border border-zinc-200 dark:border-zinc-800">
            Total: <strong className="text-zinc-900 dark:text-white">{dadosOrdens.total}</strong> ordens
          </span>
        </div>
        ...
        <div className="block md:hidden space-y-3">
          {loadingOrdens ? (
            <div className="p-8 text-center bg-zinc-50 dark:bg-zinc-950/60 rounded-xl border border-zinc-200 dark:border-zinc-800">
              ...
              <span className="text-xs text-zinc-500 dark:text-zinc-400 uppercase font-bold">Carregando ordens...</span>
            </div>
            ...
            listaOrdens.map(os => {
              ...
              return (
                <div key={os.id} className="bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 space-y-3 shadow-md transition-colors">
                  ...
                  <div className="grid grid-cols-3 gap-2 bg-white dark:bg-zinc-900/90 p-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 text-xs items-center">
                    <div>
                      <span className="text-[10px] text-zinc-500 block uppercase font-medium">Total</span>
                      <span className="font-bold text-zinc-900 dark:text-white">R$ {Number(os.total || 0).toFixed(2)}</span>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="0.00"
                        className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 rounded p-1 text-xs text-rose-600 dark:text-rose-400 font-bold focus:border-red-500 outline-none"
                      />
                    </div>

                    <div>
                      <span className="text-[10px] text-zinc-500 block uppercase font-medium">Lucro</span>
                      <span className="font-black text-emerald-600 dark:text-emerald-400">R$ {lucroCalc.toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-end pt-1">
                    <button
                      ...
                      className="w-full py-2 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-800 dark:text-white rounded-lg text-xs font-bold transition flex items-center justify-center gap-1.5 border border-zinc-300 dark:border-zinc-700 active:scale-95"
                    >
                      <Eye className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400" /> Ver Pedido Completo
                    </button>
                  </div>
                </div>
              );
        ...
        <div className="hidden md:block overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-950/40">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/60">
              {loadingOrdens ? (
                <tr>
                  <td colSpan="11" className="p-10 text-center text-zinc-500 dark:text-zinc-400">
                    <Loader2 className="w-7 h-7 text-red-500 animate-spin mx-auto mb-2" />
                    <span className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                ...
                listaOrdens.map(os => {
                  ...
                  return (
                    <tr key={os.id} className="hover:bg-zinc-100/80 dark:hover:bg-zinc-800/40 transition">
                      ...
                      <td className="p-3.5 font-bold text-zinc-900 dark:text-white">R$ {Number(os.total || 0).toFixed(2)}</td>
                      ...
                      <td className="p-3.5">
                        <div className="relative flex items-center">
                          <input
                            ...
                            onBlur={() => handleSalvarCusto(os.id, custosLocais[os.id])}
                            onKeyDown={e => e.key === 'Enter' && handleSalvarCusto(os.id, custosLocais[os.id])}
                            className="w-24 bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 hover:border-zinc-400 dark:hover:border-zinc-500 focus:border-red-500 rounded-lg px-2 py-1 text-xs text-rose-600 dark:text-rose-400 font-bold outline-none transition"
                            placeholder="0.00"
                          />
        ...
        {!loadingOrdens && dadosOrdens.total > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 text-xs text-zinc-500 dark:text-zinc-400">
            <div className="text-center sm:text-left">
              Página <span className="font-bold text-zinc-900 dark:text-white">{dadosOrdens.pagina}</span> de{' '}
              <span className="font-bold text-zinc-900 dark:text-white">{dadosOrdens.total_paginas}</span>
            </div>
